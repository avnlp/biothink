...
def extract_context(output_text):
    """Extract context from <paragraph> tags."""
    context_match = re.search(r"<paragraph>(.*?)</paragraph>", output_text, re.DOTALL)
    if not context_match:
        return None, output_text[len("[Retrieval]") :].strip()
    return context_match.group(1).strip(), output_text[context_match.end() :].strip()


def extract_relevance(remaining_text):
    """Extract relevance token from text."""
    for token in RELEVANCE_TOKENS:
        if remaining_text.startswith(token):
            return token, remaining_text[len(token) :].strip()
    return None, remaining_text


def find_next_token(text, tokens):
    """Find the first occurring token in text and return its position and value."""
    positions = [(text.find(token), token) for token in tokens]
    valid_positions = [(pos, token) for pos, token in positions if pos != -1]
    return min(valid_positions, key=lambda x: x[0]) if valid_positions else (None, None)
...
def format_output_for_dataset(example):
    ...
    if not output_text.startswith("[Retrieval]"):
        result.update(extracted_fields)
        return result

    # Extract context
    extracted_fields["context"], remaining = extract_context(output_text)

    # Extract relevance
    extracted_fields["relevance"], remaining = extract_relevance(remaining)

    # Find next token and process accordingly
    next_token_pos, next_token = find_next_token(remaining, ALL_TOKENS)

    if next_token_pos is not None:
        extracted_fields["answer"] = remaining[:next_token_pos].strip()
        remaining = remaining[next_token_pos + len(next_token) :].strip()

        if extracted_fields["relevance"] == "[Relevant]":
            if next_token in GROUNDNESS_TOKENS:
                extracted_fields["groundness"] = next_token
                # Look for utility token in remaining text
                for util_token in UTILITY_TOKENS:
                    if util_token in remaining:
                        extracted_fields["utility"] = util_token
                        break
            elif next_token in UTILITY_TOKENS:
                extracted_fields["utility"] = next_token
        elif extracted_fields["relevance"] == "[Irrelevant]" and next_token in UTILITY_TOKENS:
            extracted_fields["utility"] = next_token
    else:
