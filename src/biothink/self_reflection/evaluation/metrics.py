...
This module provides a comprehensive evaluation framework for assessing LLM outputs that follow a specific XML
structure with reasoning, context-grading, answer-utility, groundness.
...
def evaluate_relevance(extracted: str, expected: str) -> float:
    ...
    """
    return 0.0 if not extracted or not expected else 1.0 if extracted.strip() == expected.strip() else 0.0
...
def evaluate_answer_correctness(question: str, answer: str, expected_answer: str) -> float:
    ...
    return correctness_metric.score if correctness_metric.score is not None else 0.0
...
def evaluate_faithfulness(question: str, answer: str, context: str) -> float:
    ...
    return faithfulness_metric.score if faithfulness_metric.score is not None else 0.0
...
def evaluate_answer_relevancy(question: str, answer: str) -> float:
    ...
    return relevancy_metric.score if relevancy_metric.score is not None else 0.0
...
def evaluate_generation(data: dict) -> dict:
    ...
    metrics["relevance_correct"] = evaluate_relevance(extracted["relevance"], data.get("relevance") or "")
    metrics["utility_correct"] = evaluate_utility(extracted["utility"], data.get("utility") or "")
    metrics["groundness_correct"] = evaluate_groundness(extracted["groundness"], data.get("groundness") or "")
