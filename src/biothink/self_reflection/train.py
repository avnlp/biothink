...
This script implements a full workflow for fine-tuning LLMs using GRPO for bio-medical reasoning question-answering.
...
from biothink.self_reflection.prompts import SYSTEM_PROMPT, USER_PROMPT
from biothink.self_reflection.reward_functions import (
...
if __name__ == "__main__":
    ...
    dataset = dataset.map(
        lambda x: {
            "prompt": [
                ...
                {
                    "role": "user",
                    "content": USER_PROMPT.format(question=x["instruction"], context=x["context"]),
                },
            ],
        }
    )
    ...
    trainer = GRPOTrainer(
        ...
        reward_funcs=[
            correctness_reward_func,  # type: ignore[list-item]
            xml_structure_reward_func,  # type: ignore[list-item]
            utility_reward_func,
            relevance_reward_func,
            groundness_reward_func,
        ],
